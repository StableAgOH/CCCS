...
from src.network import get, post
...
for page in range(1, pages + 1):
    time.sleep(1)
    standings = get(DOMAIN + f"gym/{gym_id}/standings/page/{page}", timeout=5)
...
json.dump(teams, open(f"teams.json", "w", encoding="UTF-8"), ensure_ascii=False)
submissions.sort(key=lambda x: x[2])
json.dump(
    submissions,
    open(f"submissions.json", "w", encoding="UTF-8"),
    ensure_ascii=False,
)
